...
from neural_methods.model.MLLMTrack.add_sunlight import add_sunlight
import cv2
import numpy as np
import pandas as pd
...
class BaseLoader(Dataset):
    ...
    def __init__(self, dataset_name, raw_data_path, config_data, is_multi = True):
        ...
        """
        self.inputs = list()
        self.inputs_delete = list()
        self.labels = list()
        self.dataset_name = dataset_name
        self.raw_data_path = raw_data_path
        self.cached_path = config_data.CACHED_PATH
        self.file_list_path = config_data.FILE_LIST_PATH
        # todo: 构造multi-source的data
        if len(dataset_name) > 5: # 意味着是train1 ... multi_source
            cur = int(dataset_name[-1]) - 1
            # print(cur)
            if config_data.DATASET == "UBFC-rPPG" and config_data.MULTI_SOURCE.SOURCE_NAME[cur] != "UBFC-rPPG":
                self.file_list_path = config_data.FILE_LIST_PATH.replace("UBFC-rPPG", config_data.MULTI_SOURCE.SOURCE_NAME[cur]).replace("UBFC", config_data.MULTI_SOURCE.SOURCE_NAME[cur])
            elif config_data.DATASET != "UBFC-rPPG" and config_data.MULTI_SOURCE.SOURCE_NAME[cur] == "UBFC-rPPG":
                self.file_list_path = config_data.FILE_LIST_PATH.replace(config_data.DATASET, "UBFC").replace("UBFC_", "UBFC-rPPG_")
            elif config_data.DATASET == config_data.MULTI_SOURCE.SOURCE_NAME[cur]:
                pass
            else:
                self.file_list_path = config_data.FILE_LIST_PATH.replace(config_data.DATASET, config_data.MULTI_SOURCE.SOURCE_NAME[cur])
            # print(self.file_list_path)
        #
        # if dataset_name == "train1":
        #     if config_data.DATASET == "UBFC-rPPG":
        #         # self.cached_path = config_data.CACHED_PATH.replace("UBFC", "PURE")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("UBFC-rPPG", "PURE").replace("UBFC", "PURE")
        #     elif config_data.DATASET == "PURE":
        #         # self.cached_path = config_data.CACHED_PATH.replace("PURE", "UBFC")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("PURE", "UBFC").replace("UBFC_", "UBFC-rPPG_")
        #     elif config_data.DATASET == "MMPD":
        #         # self.cached_path = config_data.CACHED_PATH.replace("MMPD", "UBFC")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("MMPD", "UBFC").replace("UBFC_", "UBFC-rPPG_")
        #     elif config_data.DATASET == "BUAA":
        #         # self.cached_path = config_data.CACHED_PATH.replace("BUAA", "UBFC")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("BUAA", "UBFC").replace("UBFC_", "UBFC-rPPG_")
        # elif dataset_name == "train2":
        #     if config_data.DATASET == "UBFC-rPPG":
        #         # self.cached_path = config_data.CACHED_PATH.replace("UBFC", "MMPD")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("UBFC-rPPG", "MMPD").replace("UBFC", "MMPD")
        #     elif config_data.DATASET == "PURE":
        #         # self.cached_path = config_data.CACHED_PATH.replace("PURE", "MMPD")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("PURE", "MMPD")
        #     elif config_data.DATASET == "MMPD":
        #         # self.cached_path = config_data.CACHED_PATH.replace("MMPD", "PURE")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("MMPD", "PURE")
        #     elif config_data.DATASET == "BUAA":
        #         # self.cached_path = config_data.CACHED_PATH.replace("BUAA", "PURE")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("BUAA", "PURE")
        # elif dataset_name == "train3":
        #     if config_data.DATASET == "UBFC-rPPG":
        #         # self.cached_path = config_data.CACHED_PATH.replace("UBFC", "BUAA")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("UBFC-rPPG", "BUAA").replace("UBFC", "BUAA")
        #     elif config_data.DATASET == "PURE":
        #         # self.cached_path = config_data.CACHED_PATH.replace("PURE", "BUAA")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("PURE", "BUAA")
        #     elif config_data.DATASET == "MMPD":
        #         # self.cached_path = config_data.CACHED_PATH.replace("MMPD", "BUAA")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("MMPD", "BUAA")
        #     elif config_data.DATASET == "BUAA":
        #         # self.cached_path = config_data.CACHED_PATH.replace("BUAA", "MMPD")
        #         self.file_list_path = config_data.FILE_LIST_PATH.replace("BUAA", "MMPD")
        ...
        if config_data.DO_PREPROCESS:
            ...
            # print(len(self.raw_data_dirs)) # 0
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name == "train":
            # data, strength = add_sunlight(data)
            strength = 0
        else:
            strength = 0
        # if self.config_data.DATASET == "BUAA":

        data_delete = np.load(self.inputs_delete[index])
        label = np.load(self.labels[index])
        if self.data_format == 'NDCHW':
            data = np.transpose(data, (0, 3, 1, 2))
            data_delete = np.transpose(data_delete, (0, 3, 1, 2))
        elif self.data_format == 'NCDHW':
            data = np.transpose(data, (3, 0, 1, 2))
            data_delete = np.transpose(data_delete, (3, 0, 1, 2))
        elif self.data_format == 'NDHWC':
            pass
        else:
            raise ValueError('Unsupported Data Format!')
        data = np.float32(data)
        data_delete = np.float32(data_delete)
        ...
        return data, label, filename, chunk_id, data_delete, strength
    ...
    def crop_face_resize(self, frames, use_face_detection, backend, use_larger_box, larger_box_coef, use_dynamic_detection,
        ...
        # Frame Resizing
        resized_frames = np.empty((frames.shape[0], height, width, 3), dtype=frames.dtype)
    ...
    def multi_process_manager(self, data_dirs, config_preprocess, multi_process_quota=1):
        ...
        pbar = tqdm(total=file_num, desc=f"Preprocess {self.dataset_name}", dynamic_ncols=True)
        ...
        p_list = []  # list of tuples: (process_obj, data_index)
        ...
        for i in choose_range:
            ...
            while process_flag:  # ensure that every i creates a process
                if running_num < multi_process_quota:  # in case of too many processes
                    ...
                    p.start()
                    try:
                        cur_index = data_dirs[i].get('index', str(i))
                    except Exception:
                        cur_index = str(i)
                    pbar.set_postfix_str(f"running: {cur_index}")
                    p_list.append((p, i))
                    running_num += 1
                    process_flag = False
                for p_, idx_ in list(p_list):
                    if not p_.is_alive():
                        p_list.remove((p_, idx_))
                        p_.join()
                        if p_.exitcode is not None and p_.exitcode != 0:
                            failed_processes.append((idx_, p_.exitcode))
                        running_num -= 1
                        pbar.update(1)
        # join all processes
        for p_, idx_ in p_list:
            p_.join()
            if p_.exitcode is not None and p_.exitcode != 0:
                failed_processes.append((idx_, p_.exitcode))
            pbar.update(1)
        ...
        if failed_processes:
            failed_processes = sorted(failed_processes, key=lambda x: x[0])
            raise RuntimeError(
                f"{self.dataset_name} preprocessing failed for {len(failed_processes)} items; "
                f"failed indices/exitcodes: {failed_processes}. "
                f"This is commonly caused by OOM-kill or subprocess exceptions."
            )
    ...
    def load_preprocessed_data(self):
        ...
        # todo:加了raw数据集
        # file_list_path_delete = file_list_path.replace("DiffNormalized", "Raw")
        file_list_path_delete = self.file_list_path

        # if file_list_path[-5] == "0":
        #     file_list_path_delete = "/public/home/cit_yipingxie/dataset3/PURE/DataFileLists/PURE_SizeW128_SizeH128_ClipLength128_DataTypeRaw_DataAugNone_LabelTypeRaw_Crop_faceTrue_BackendHC_Large_boxTrue_Large_size1.5_Dyamic_DetFalse_det_len30_Median_face_boxFalse_0.0_1.0.csv"
        # else:
        #     file_list_path_delete = "/public/home/cit_yipingxie/dataset3/PURE/DataFileLists/PURE_SizeW128_SizeH128_ClipLength128_DataTypeRaw_DataAugNone_LabelTypeRaw_Crop_faceTrue_BackendHC_Large_boxTrue_Large_size1.5_Dyamic_DetFalse_det_len30_Median_face_boxFalse_0.0_0.72.csv"
        file_list_df = pd.read_csv(file_list_path)
        file_list_df_delete = pd.read_csv(file_list_path_delete)
        inputs = file_list_df['input_files'].tolist()
        inputs_delete = file_list_df_delete['input_files'].tolist()
        if not inputs:
            raise ValueError(self.dataset_name + ' dataset loading data error!')
        if not inputs_delete:
            raise ValueError(self.dataset_name + ' dataset loading data error!')
        inputs = sorted(inputs)  # sort input file name list
        inputs_delete = sorted(inputs_delete)
        labels = [input_file.replace("input", "label") for input_file in inputs]
        self.inputs_delete = inputs_delete
        # print('输入的长度： ', len(self.inputs_delete))
        self.inputs = inputs
        self.labels = labels
    ...
    @staticmethod
    def resample_ppg2(input_signal, target_length):
        Time = np.linspace(0, target_length, len(input_signal))
        CSI_Time = np.linspace(0, target_length, target_length)
        t = interpolate.splrep(Time, input_signal)
