...
_C.TRAIN.TWO_STAGE_TRAINING = False
_C.TRAIN.STAGE1_EPOCHS = 10
_C.TRAIN.STAGE2_EPOCHS = 10
_C.TRAIN.STAGE1_LR = 1e-3
_C.TRAIN.STAGE2_LR = 5e-5
# Three-stage training settings
_C.TRAIN.TRAINING_STAGE = 'stage1'
_C.TRAIN.STAGE1_CHECKPOINT = ''
_C.TRAIN.STAGE2_CHECKPOINT = ''
_C.TRAIN.STAGE3_CHECKPOINT = ''
_C.TRAIN.STAGE3_EPOCHS = 20
_C.TRAIN.STAGE3_LR = 1e-5
# Train.Data settings
_C.TRAIN.DATA = CN()
...
_C.TRAIN.DATA.MULTI_SOURCE = CN()
_C.TRAIN.DATA.MULTI_SOURCE.NUM_SOURCE = 0
_C.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME = []
...
_C.TRAIN.DATA.PREPROCESS.VIDEO_TYPE = 'RGB'  # For Zhuhai dataset: 'RGB', 'IR1', or 'IR2'
...
_C.TRAIN.DATA.PREPROCESS.PROMPT_GENERATION = CN()
_C.TRAIN.DATA.PREPROCESS.PROMPT_GENERATION.IS_PREPROCESS = False
_C.TRAIN.DATA.PREPROCESS.PROMPT_GENERATION.PREPROCESS_PATH = []
# _C.TRAIN.DATA.PREPROCESS.PROMPT_GENERATION.PREPROCESS_PATH = ""
...
_C.VALID.DATA.PREPROCESS.VIDEO_TYPE = 'RGB'  # For Zhuhai dataset: 'RGB', 'IR1', or 'IR2'
...
_C.TEST.DATA.PREPROCESS.VIDEO_TYPE = 'RGB'  # For Zhuhai dataset: 'RGB', 'IR1', or 'IR2'
...
_C.TEST.DATA.PREPROCESS.PROMPT_GENERATION = CN()
_C.TEST.DATA.PREPROCESS.PROMPT_GENERATION.IS_PREPROCESS = False
_C.TEST.DATA.PREPROCESS.PROMPT_GENERATION.PREPROCESS_PATH = ""
...
# -----------------------------------------------------------------------------
# Model Settings for PhysLLM
# -----------------------------------------------------------------------------
_C.MODEL.FACE_MODEL = None
_C.MODEL.ENV_MODEL = None
_C.MODEL.VIDEO_ENC = None
_C.MODEL.LLM = None
_C.MODEL.MLLM = None

# -----------------------------------------------------------------------------
# Model Settings for PhaseNet
# -----------------------------------------------------------------------------
_C.MODEL.PHASENET = CN()
_C.MODEL.PHASENET.PARAMS = CN()
_C.MODEL.PHASENET.PARAMS.FEATURE_DIM = 256
_C.MODEL.PHASENET.PARAMS.LATENT_DIM = 64
_C.MODEL.PHASENET.PARAMS.HIDDEN_DIM = 256
_C.MODEL.PHASENET.PARAMS.TCN_LAYERS = 4
_C.MODEL.PHASENET.PARAMS.SCALE_NAME = "base"
_C.MODEL.PHASENET.PARAMS.ENCODER_CHANNELS = [16, 32, 64, 128]
_C.MODEL.PHASENET.PARAMS.ENCODER_EXPAND_RATIO = 4
_C.MODEL.PHASENET.PARAMS.TEMPORAL_MODULE = "gated_tcn"
_C.MODEL.PHASENET.PARAMS.PHASE_FS = 30.0
_C.MODEL.PHASENET.PARAMS.PHASE_LOW_BPM = 45.0
_C.MODEL.PHASENET.PARAMS.PHASE_HIGH_BPM = 150.0
_C.MODEL.PHASENET.PARAMS.PHASE_NUM_FREQ_BINS = 96
_C.MODEL.PHASENET.PARAMS.PHASE_DROPOUT = 0.1
_C.MODEL.PHASENET.PARAMS.K_STEPS = 10
_C.MODEL.PHASENET.PARAMS.K_LR = 0.1

# -----------------------------------------------------------------------------
# Model Settings for QwenVL-rPPG
# -----------------------------------------------------------------------------
_C.MODEL.QWEN_MODEL_NAME = ''
_C.MODEL.USE_LORA = False
_C.MODEL.LORA_R = 16
_C.MODEL.LORA_ALPHA = 32
_C.MODEL.LORA_DROPOUT = 0.1
_C.MODEL.FREEZE_VISION_LAYERS = 0
_C.MODEL.FREEZE_VISION = False
_C.MODEL.FREEZE_LLM = True
_C.MODEL.TASK_LIST = []
_C.MODEL.MAIN_TASK = ''
# Stage-specific configurations
_C.MODEL.STAGE1 = CN()
_C.MODEL.STAGE1.USE_HARD_NEGATIVES = False
_C.MODEL.STAGE1.HARD_NEGATIVE_RATIO = 0.3
_C.MODEL.STAGE1.PERIODICITY_THRESHOLD = 1.5
_C.MODEL.STAGE2 = CN()
_C.MODEL.STAGE2.LOSS_TYPE = 'neg_pearson'
_C.MODEL.STAGE2.USE_FREQUENCY_LOSS = False
_C.MODEL.STAGE2.FREQ_LOSS_WEIGHT = 0.3
_C.MODEL.STAGE3 = CN()
_C.MODEL.STAGE3.TASK_SAMPLING = 'uniform'
_C.MODEL.STAGE3.TASK_WEIGHTS = CN()
_C.MODEL.STAGE3.TASK_WEIGHTS.heart_rate = 1.0
_C.MODEL.STAGE3.TASK_WEIGHTS.respiration = 1.0
_C.MODEL.STAGE3.TASK_WEIGHTS.hrv = 1.0
_C.MODEL.STAGE3.TASK_WEIGHTS.spo2 = 1.0
...
# Generation config for LLM-based inference
_C.INFERENCE.GENERATE_MODE = False
_C.INFERENCE.MAX_NEW_TOKENS = 20
_C.INFERENCE.TEMPERATURE = 0.0
_C.INFERENCE.DO_SAMPLE = False

# -----------------------------------------------------------------------------
# Optimization settings
# -----------------------------------------------------------------------------
_C.OPTIMIZATION = CN()
_C.OPTIMIZATION.GRADIENT_ACCUMULATION_STEPS = 1
_C.OPTIMIZATION.USE_AMP = False
_C.OPTIMIZATION.AMP_DTYPE = 'float16'
_C.OPTIMIZATION.MAX_GRAD_NORM = 1.0
_C.OPTIMIZATION.LR_SCHEDULER = 'constant'
_C.OPTIMIZATION.WARMUP_EPOCHS = 0
_C.OPTIMIZATION.WEIGHT_DECAY = 0.0
_C.OPTIMIZATION.EARLY_STOPPING = CN()
_C.OPTIMIZATION.EARLY_STOPPING.ENABLED = False
_C.OPTIMIZATION.EARLY_STOPPING.PATIENCE = 10
_C.OPTIMIZATION.EARLY_STOPPING.MIN_DELTA = 0.0
...
def update_config(config, args):
    ...
    if config.TOOLBOX_MODE == 'train_and_test' or config.TOOLBOX_MODE == 'only_test' or config.TOOLBOX_MODE == 'multi_train_and_test' or config.TOOLBOX_MODE == 'salency_map':
